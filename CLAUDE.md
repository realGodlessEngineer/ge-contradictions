...
- **Git**: this directory **is** a git repository (default branch `main`) with a `.gitignore` that excludes `node_modules/`, Python bytecode (`__pycache__/`), all `*.db` binaries (`contradictions.db` is rebuilt from `db/schema.sql`+`db/data.sql`, so it is not tracked), the `.archive/` backup snapshots (`*.bak-*`), and `.env`.
