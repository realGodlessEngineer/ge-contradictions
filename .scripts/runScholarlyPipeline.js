/**
 * runScholarlyPipeline.js — Generator → auditor loop over data/scholarly/batch_*.json
 * using the Anthropic Message Batches API (50% cost discount).
 *
 *   PING=1 node runScholarlyPipeline.js     # validate CLAUDE_API_KEY + model, then exit (cheap)
 *   node runScholarlyPipeline.js            # run the loop
 *
 * Loop (capped at 2 audit round-trips per the project decision):
 *   pass 1: GENERATE every entry  → AUDIT every entry
 *   pass 2: re-GENERATE flagged entries (with auditor feedback) → re-AUDIT them
 *   stop. Any entry still carrying a medium-or-higher issue is marked needs_human
 *   and recorded in data/scholarly/UNRESOLVED.md.
 *
 * Resumable: the in-flight batch id is persisted to data/scholarly/_state.json, so a
 * re-run after a crash polls that batch and applies its results before continuing.
 * Results are matched back to entries by the `id` inside each tool output (not by
 * request position), so resume is robust.
 *
 * Env: DIR (./data/scholarly), MODEL (claude-opus-4-8), MAX_TOKENS (8000),
 *      BATCH_SIZE (20), MAX_AUDIT_PASSES (2), POLL_SECONDS (30)
 */
const fs = require('fs');
...
function applyGen(results, byId) {
    ...
    for (const cid of Object.keys(results)) {
        for (const a of (results[cid].input.assessments || [])) {
            const e = byId.get(a.id);
            if (!e) continue;
            e.assessment = {
                scholarly_consensus: a.scholarly_consensus,
                textual_variant_involved: !!a.textual_variant_involved,
                variant_description: a.variant_description || '',
                critical_apparatus_ref: a.critical_apparatus_ref || '',
                reasoning: a.reasoning || '',
            };
            e._audit = null; // freshly (re)assessed — prior audit is stale
            n++;
        }
    }
    return n;
...
function writeUnresolvedReport(batches, unresolved) {
    ...
    const lines = ['# Unresolved scholarly-assessment entries (medium-or-higher issues after 2 audit passes)', ''];
...
(async () => {
    ...
    if (!fs.existsSync(DIR)) { console.error(`No ${DIR} — run exportScholarlyBatches.js first.`); process.exit(1); }
    ...
    while (state.completedAuditPasses < MAX_AUDIT_PASSES) {
        ...
        const toGen = entries.filter(e => !e.assessment || (e._audit && e._audit.status === 'issues'));
        ...
        const toAudit = entries.filter(e => e.assessment && (!e._audit || e._audit.status === 'issues'));
    ...
    // ---- summary ----
    const have = entries.filter(e => e.assessment);
    ...
    const noAssessment = entries.filter(e => !e.assessment);
    console.log('\n========== PIPELINE SUMMARY ==========');
    console.log(`entries: ${entries.length}`);
    console.log(`  assessed: ${have.length}`);
    ...
    if (noAssessment.length) console.log(`  NO assessment (batch errors): ${noAssessment.length} -> ${noAssessment.map(e => e.id).join(', ')}`);
    console.log(`Mergeable now: clean + clean_low = ${clean.length + lowOnly.length}.  Recorded in ${path.join(DIR, 'UNRESOLVED.md')}.`);
    console.log('Next: node mergeScholarlyToDb.js');
